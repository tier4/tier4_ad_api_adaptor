...
| 条件              | 説明                                                   |
| ----------------- | ------------------------------------------------------ |
| Lv2自動運転       | 介入責任者に割り当てられた介入オペレーターが存在する。 |
| Lv3自動運転(参考) | 介入オペレーターが一人以上存在する。                   |
| Lv4自動運転       | 対応オペレーターが一人以上存在する。                   |
...
- 車両は要求する自動運転レベルに従って動作を変更する。
  ...
  - 車両がLv2に設定されている場合、介入責任者が存在する場合に自動走行可能と定義する。
  - 車両がLv4に設定されている場合、対応オペレーターが存在する場合に自動走行可能と定義する。
  - 車両が自動走行可能ではない場合、自動運転状態になければ自動運転の開始要求を拒否する。
  - 車両が自動走行可能ではない場合、自動運転状態であれば緩減速により停止し、自動運転状態を解除する。
...
- 介入実施状態についてはAD APIのManual Control APIを使用する。
  - 上記とは別に初期設定と専用のハートビードとコマンドの送信が必要となる。
